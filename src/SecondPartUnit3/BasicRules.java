...
public class BasicRules {
    ...
    /*
    就开始面向对象了。
    基本知识：.java文件只要有主方法就可以单独运行，这点和python很像，可以把它当模块用，可以把它当程序入口。
    每一个.java文件都只能有一个public的类，且这个类必须和文件名相同。
    1实例化：就是按照一个模版（类），创建一个对象，有点类似于c里的结构体，但是c里的结构体内部的元素默认全部public，没有封装性。且这些对象都是分配在
    heap上的，常规咱们写数据结构里的结构体都是全局变量，你声明一个，一般不用malloc的话，就在静态内存区域（编译的时候确定的）
    我后面已经写了一个经典的，调用带参数的构造函数的。这构造函数只要是传入参数不同我就可以一直重载.其他方法也是这样的，疯狂重载就行
